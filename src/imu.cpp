...
//读取生成的imu
// 数据并用imu动力学模型对数据进行计算，最后保存imu积分以后的轨迹，
//用来验证数据以及模型的有效性。
void IMU::testImu(std::string src, std::string dist)
{
    ...
    for (int i = 1; i < imudata.size(); ++i) {
        ...
        //Euler Integral for theta
        // Eigen::Vector3d dtheta_half =  imupose.imu_gyro * dt /2.0;
        
        //mid-point integral for theta
        ...
        /// 中值积分
        Eigen::Vector3d dtheta_half =  ((imupose_pre.imu_gyro + imupose.imu_gyro) / 2.0) * dt /2.0;
        dq.w() = 1;
        dq.x() = dtheta_half.x();
        dq.y() = dtheta_half.y();
        dq.z() = dtheta_half.z();
        Eigen::Vector3d acc_w = (Qwb*imupose_pre.imu_acc + (Qwb*=dq)*imupose.imu_acc + 2*gw) / 2;  // aw = Rwb * ( acc_body - acc_bias ) + gw
        Vw = Vw + acc_w * dt;
        Pwb = Pwb + Vw * dt + 0.5 * dt * dt * acc_w;
